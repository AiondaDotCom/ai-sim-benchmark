...
### OpenCode — Qwen 3.8 27B (local, Mac Studio M3 Ultra, 256 GB RAM)

Recorded 2026-08-25 with the frozen prompt. Needed **no corrective interventions** — nothing was broken — but four or more follow-up prompts asked for stronger visual effects, so the recorded look is partly operator-directed rather than agent-chosen (details in [INTERVENTIONS.md](INTERVENTIONS.md)). It is the first run in this repository generated by a **locally hosted model** — no API, no cloud inference: Qwen 3.8 27B running on a Mac Studio M3 Ultra with 256 GB of RAM (unified memory). The smallest model recorded here by parameter count, and the only run that produced a **caldera island** rather than an open mountain slope: a sealed rim wall funnels rain, three mountain springs, and a carved river channel into one central lake, so the water budget stays visibly closed and evaporation holds the lake level steady. 19/19 tests (terrain determinism, mass conservation to < 1e-4, downhill flow, long-run stability), clean `tsc` build, and a rendered scene verified in the browser: shadowed caldera walls, a depth-faded lake with a sun glint, rain, drifting clouds, and a ~115 s camera orbit.
...
Passed after 25 corrective interventions and 17 requirement changes (2026-08-23) — by far the most heavily iterated run in this repository, and the one that says the most about how these agents fail. The first working version arrived in ~54 minutes and was sound underneath: deterministic simulation, clean layering, a working asset pipeline. Almost everything afterwards was visual fidelity, and almost every defect was found by a human comparing the rendered result against the reference, not by the agent's own verification — which was rigorous by this benchmark's standards, with measurement harnesses and permanent tests, and still passed a white showroom for a film-noir lobby, elevator cabs behind solid geometry, tile seams drawn through stripped areas, bullet holes floating on stone that had been shot away, damage bulging outward so a shot column grew *wider*, hands rotated a quarter turn on their grips, a camera that shook continuously, a bullet-cam that never showed a bullet, dead soldiers sitting to attention, and bodies sunk half into the marble.

Final state: a dark teal-graded granite hall where the cladding is a real second layer that fractures into irregular plates, exposes a rough core with genuine depth, chews the column silhouettes, and occasionally lets a whole tile crash down and shatter; a full post-processing stack; film-style ballistics with modeled spinning bullets, a bullet-cam and air-wake trails; green-uniformed military reinforcements who flood the hall, take cover and shout a command into a held silence; English voice acting; an industrial metal-percussion score whose drop lands on the eruption to the millisecond. 74 tests including scene-wide invariants — no body inside the set at any frame, none below the floor, no slow-motion window outliving its shot, every audio cue resolving to a file that exists. Fully offline, no API keys ever committed. Details in [INTERVENTIONS.md](INTERVENTIONS.md); the architecture question it raised is answered in the run's own `ARCHITECTURE-REVIEW.md`.

![Claude Code Fable 5 lobby shootout demo](runs-lobby/claude-code-fable-5/docs/demo.gif)

*(GIF plays at 4.5× speed; the recording runs 95 s, cut on the closing hold. Captured with `?loop=0` so it ends there rather than restarting; the full sequence runs a little longer in the demo itself.)*
...
| OpenCode — Qwen 3.8 27B (local) | frozen prompt from PROMPT.md verbatim |
