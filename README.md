...
Passed after 11 corrective interventions, and then extended far beyond the frozen prompt (2026-08-23). The run began as a fast, clean delivery — first working version in ~54 minutes — and became the most heavily iterated run in this repository: twelve requirement changes (A1–A12) and eleven corrective interventions (B1–B11), driven by an operator reviewing the rendered result against the original film scene frame by frame. The arc is the interesting part: the simulation, tests and asset pipeline were sound from the first delivery, while nearly every correction concerned visual fidelity to the reference — a look that was too bright and clinical, elevator doors that vanished inside a granite retexture, single-layer damage where the brief called for cladding over substrate, a metal detector without an alarm lamp, weapons that never accumulated on the floor, characters that read as toys. Final state: a dark teal-graded granite hall with two-layer destruction that strips whole tiles and chews column silhouettes, a full post-processing stack, film-style ballistics with modeled bullets and a bullet-cam, English voice acting, and a heavy-metal score cut to the choreography. 34 tests, 60 fps, fully offline, no API keys ever committed. Details in [INTERVENTIONS.md](INTERVENTIONS.md); the architecture question it raised is answered in the run's own `ARCHITECTURE-REVIEW.md`.

![Claude Code Fable 5 lobby shootout demo](runs-lobby/claude-code-fable-5/docs/demo.gif)

*(GIF plays at 5× speed; the sequence runs ~98 s in real time.)*
